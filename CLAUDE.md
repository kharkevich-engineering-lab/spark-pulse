...
Deployments run natively only: `tools.native_runtime` drives Docker on every node through that node's agent, `tools.deployment_records` owns `deployments.json`, and `tools.deploy_dispatch` is the one entry point the routers and MCP tools call (it adapts a plan to a dict and a status read to a live one; it used to *dispatch* by the record's `runtime`, which was there for records the removed `run-recipe.sh` runner had left behind). `config.runtime` resolves anything to `native`.
...
**The agent invariant, and how it is checked.** The control plane coordinates; agents execute. Anything on a node — a container, an image, a GPU reading, a model snapshot, a process — is reached by naming that node and sending the operation to its agent, including when the node is the machine this process runs on (`service_for` has no local branch; it reaches its own agent over loopback). `tests/test_no_local_operations.py` is the ratchet, and it reads the source because the property is *which module a call goes to*: no tool may call `docker._get_service()`, shell out, write to this machine's filesystem, or signal a process unless it is named in an allowlist with its reason, and a name that stops needing its exception has to leave. `./scripts/check-agent-only.sh` runs it and prints what is still excused, plus `STILL_SINGLE_NODE` — the operations that are not node operations *yet* (`preflight`'s host probes, `/api/cache`, `replicate_to_nodes`'s rsync, interface discovery), each with what the change would need. The list is there so the exceptions cannot be read as a clean bill of health.

**Monitoring across nodes** (`tools/node_stats.py`): `GET /api/memory` — one endpoint, answering `{"nodes": [...]}` — and `/sse/metrics` ask *every* registered node for its own stats through its own agent — `GetNodeStats` — including the machine this process runs on. `tools/system.py` and its mock are gone with the last of the local `nvidia-smi`/`free`/`df` shell-outs; the parsing lives in the agent (`agent/src/executor/stats.rs`) and its tests with it. The conversions that remain here are units (the protocol is bytes, the page reads megabytes) and absence (`memory_supported: false` is how a GB10's `[N/A]` renders, never a zero). Whether a GPU process is *ours* is decided from two halves: the node says which container the process is in, the control plane's managed-container list says which containers it started and for which deployment. `DELETE /api/memory/processes/{pid}?node=` stops that container when it is one of ours and signals the process otherwise (`TerminateProcess`), so the kill button works on a node that is not this one. A node that cannot be asked keeps its section and says why — unknown is not idle.
...
**Model presence and removal across nodes** (`tools/models.py`): `GET /api/models/{id}/presence` asks each node to *list* its snapshot through its agent (`ListSnapshot`) and reaches the verdict here, from the manifest the control plane already holds — `hub_cache.verify_listing()`. It used to ship `hub_cache.py` to every node over SSH and run it there, which is a second copy of the verifier on a machine that may not have the interpreter for it. `SnapshotFile.resolved` is what keeps a dangling symlink (a snapshot copied without its blobs) from arriving as a zero-byte file. `DELETE /api/models/{id}?nodes=a,b` removes it here and on every node named, each answering for itself; the Models page's dialog preselects the nodes presence says hold a copy. Replication (`replicate_to_nodes`) still uses SSH — it is an rsync of the bytes themselves, and it still ships the verifier because it writes the completion marker on the node.

**Mods** (`tools/mods.py`, `routers/mods.py`): listing, inspection and content validation only. Applying a mod happens on the deploy path — a recipe names its mods and `native_runtime._apply_mods` copies them into each rank's container through that rank's node service. A second implementation used to exist behind `POST /api/mods/apply|rollback`: a `ModOrchestrator` walking a `cluster_state` of head and workers, a shape the removed cluster orchestrator produced and nothing had produced since, whose only caller never sent one. Both endpoints and the orchestrator are gone.
...
Each store imports its old JSON file **once**, on first read, recorded in the `meta` table — not inferred from an empty table, because deleting the last row would re-import and resurrect what an operator removed. This is the one compatibility path kept on purpose: it is somebody's data, not somebody's code. The JSON files are left where they are.
...
`docs/` is a [Docsify](https://docsify.js.org) site: `index.html` fetches the markdown at runtime, so there is no build step and what is published is exactly what is in the tree. `docs/_sidebar.md` is the navigation — its links are **root-absolute** (`/guide/tour.md`) because `relativePath: true` otherwise resolves them against the current page's directory and every link 404s once per page. In-page links stay relative so they also work when the file is read on GitHub. Mermaid is initialised in `index.html` rather than through a plugin, so the two cannot go out of step.

`scripts/check-docs-links.py` fails on a broken relative link or a page nothing links to; `.github/workflows/pages.yml` runs it and then publishes `docs/` to GitHub Pages from CI (no `gh-pages` branch — Pages must be set to "GitHub Actions" as its source). Screenshots under `docs/assets/screenshots/` are regenerated by `node web/scripts/capture-screenshots.mjs --base http://127.0.0.1:8123` against a simulation backend; a page that changes and a screenshot that does not is documentation that lies.
