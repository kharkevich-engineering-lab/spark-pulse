...
import { Database, Trash2, Loader2, AlertCircle, FolderOpen } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      {cacheData?.entries.map((e) => (
        <div key={e.name} className="rounded-xl bg-surface border border-border p-5">
          <div className="flex items-start justify-between">
            <div className="flex items-start gap-3 flex-1">
              <FolderOpen size={20} className="text-text-muted mt-0.5 shrink-0" />
              <div className="min-w-0"><h3 className="font-semibold">{e.name}</h3><p className="text-sm text-text-muted truncate font-mono">{e.path}</p>{e.description && <p className="text-xs text-text-muted mt-1">{e.description}</p>}</div>
            </div>
            <div className="text-right shrink-0 ml-4"><p className="font-mono font-bold">{formatSize(e.size_bytes)}</p><p className="text-xs text-text-muted">{e.file_count} files</p></div>
          </div>
          <button onClick={() => setCleanTarget(e.name)} disabled={cleaning === e.name} className="mt-4 w-full py-2 rounded-lg border border-border hover:border-danger/50 hover:text-danger text-text-muted hover:bg-danger/5 transition-colors flex items-center justify-center gap-2 text-sm disabled:opacity-50">
            {cleaning === e.name ? <Loader2 className="animate-spin" size={14} /> : <Trash2 size={14} />}{cleaning === e.name ? "Cleaning..." : "Clean"}
          </button>
        </div>
      ))}
